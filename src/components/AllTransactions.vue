<template>
  <Transition>
    <div v-if="isLoaded" class="all-transactions-content">
      <select
        v-if="hasTransactions"
        class="category-input"
        v-model="transactionDuration"
      ...
      <div class="all-transactions-container" v-if="hasTransactions">
        <div
        ...
        >
          ...
          <div class="last-transactions-amount">
            ...
            <h3 v-if="transaction.type == 'income'" class="income-transaction">
              {{ formatAmount(transaction) }}
            </h3>
          </div>
        </div>
...
export default {
  ...
  methods: {
    filterTransactionsByDate() {
      ...
      if (this.transactionDuration === "all time") {
        filteredTransactions = this.transactions;
      } else if (this.transactionDuration === "month") {
      ...
      } else if (this.transactionDuration === "day") {
        filteredTransactions = this.transactions.filter((transaction) => {
          const transactionDate = new Date(transaction.date);
          return (
      ...
      this.formattedTransactions = filteredTransactions.sort((a, b) => {
  ...
  async mounted() {
    ...
    await this.filterTransactionsByDate();
    this.isLoaded = true;
  },
};
</script>
...
.all-transactions-container .last-transactions-card {
  display: flex;
  align-items: center;
